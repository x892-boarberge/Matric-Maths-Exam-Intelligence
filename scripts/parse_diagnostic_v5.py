"""
DBE diagnostic parser v5.
Handles 2015 (numbered bullets, no-colon headings),
2018 (a. bullets), 2019 (standard) with corrected ranges.
"""
import re
import pandas as pd
import pymupdf
...
OUT = ROOT / "data" / "processed" / "diagnostics" / "diagnostic_errors_v5.csv"
...
def find_question_headings(text):
    """Try each regex, return first that finds headings."""
...
def split_paras(block, year):
    """
    Year-specific bullet splitting.
    2015: 1. 2. 3.
    2018: a. b. c.
    2019+: (a) (b) (c)
    """
    if year == 2015:
        pat = re.compile(r"(?:^|\n)\s*(\d+)\.\s*\n(.+?)(?=\n\s*\d+\.\s*\n|\Z)", re.DOTALL)
        out = []
        for num, raw in pat.findall(block):
            cleaned, raw_len, clean_len = clean(raw)
            out.append((num, cleaned, raw_len, clean_len))
        return out
