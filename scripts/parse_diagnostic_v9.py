"""DBE diagnostic parser v8. Final fixes: 2015 range extension, 2019 splitter separation."""
import re
import pandas as pd
import pymupdf
...
OUT = ROOT / "data" / "processed" / "diagnostics" / "diagnostic_errors_v9.csv"

PAGE_RANGES = {
    2014: (117, 139), 2015: (155, 185), 2016: (155, 181),
...
NEXT_CHAPTER_RE = re.compile(r"CHAPTER\s+1[1-9]|PHYSICAL\s+SCIENCES|ACCOUNTING|LIFE\s+SCIENCES|ECONOMICS|TOURISM|BUSINESS", re.I)
...
# 2019 — separate splitters for errors vs suggestions
...
def split_paras_2019_errors(block):
...
def split_paras_2019_suggestions(block):
...
def split_paras_errors(block, year):
    if year == 2015:
        pat = re.compile(r"(?:^|\n)\s*(\d+)\.\s*\n?(.+?)(?=\n\s*\d+\.\s*|\Z)", re.DOTALL)
    ...
    if year == 2019:
        return split_paras_2019_errors(block)
    pat = re.compile(r"\(([a-z])\)\s+(.+?)(?=\([a-z]\)\s+|\Z)", re.DOTALL)
    return [(l, *clean(r)) for l, r in pat.findall(block)]


def split_paras_suggestions(block, year):
    if year == 2015:
        pat = re.compile(r"(?:^|\n)\s*(\d+)\.\s*\n?(.+?)(?=\n\s*\d+\.\s*|\Z)", re.DOTALL)
        return [(n, *clean(r)) for n, r in pat.findall(block)]
    if year == 2018:
        ...
        return [(l, *clean(r)) for l, r in pat.findall(block)]
    if year == 2019:
        return split_paras_2019_suggestions(block)
    pat = re.compile(r"\(([a-z])\)\s+(.+?)(?=\([a-z]\)\s+|\Z)", re.DOTALL)
    return [(l, *clean(r)) for l, r in pat.findall(block)]
...
def parse_section(year, paper_num, section_text):
    ...
    for i, m in enumerate(q_matches):
        ...
        for letter, body, raw_len, clean_len in split_paras_errors(err_body, year):
            if len(body) < 50:
                continue
        ...
        for letter, body, raw_len, clean_len in split_paras_suggestions(sugg_body, year):
            if len(body) < 40:
                continue
            rows.append({
                "year": year, "paper": f"P{paper_num}",
