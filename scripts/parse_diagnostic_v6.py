"""
DBE diagnostic parser v6.
- 2019: paragraph-based split (bullets are layout-scrambled in source)
- Reports UNMAPPED rows for audit
- Target: 100% coverage, 0 unmapped
"""
import re
import pandas as pd
import pymupdf
...
OUT = ROOT / "data" / "processed" / "diagnostics" / "diagnostic_errors_v6.csv"

PAGE_RANGES = {
    2014: (117, 139), 2015: (155, 176), 2016: (155, 181),
    2017: (161, 184), 2018: (139, 153), 2019: (177, 203),
...
# ---------- 2019 special: paragraph-based splitting ----------
PARA_START_RE = re.compile(
    r"(?=(?:In\s+Q\d|In\s+answering|Many\s+candidates|Some\s+candidates|"
    r"Most\s+candidates|Candidates|A\s+few\s+candidates|Writing\s+down|"
    r"The\s+majority|Learners|Very\s+few|Others|This\s+question))",
    re.I,
)

def split_paras_2019(block):
    """2019: bullets are scrambled. Split on paragraph starts instead."""
    block = re.sub(r"\([a-z]\)\s*\n?", " ", block)  # strip stray bullets
    # Normalize newlines
    block = re.sub(r"[ \t]+", " ", block)
    # Split on paragraph-start keywords
...
def main():
    ...
    print(f"Diagram-assumption: {int(errs['diagram_assumption'].sum())}")
    print()
    # UNMAPPED audit
    unmapped = errs[errs["topic"] == "UNMAPPED"]
    print(f"UNMAPPED rows: {len(unmapped)}")
    if len(unmapped) > 0:
        print()
        print("Full text of UNMAPPED rows (for audit):")
        for i, (_, r) in enumerate(unmapped.iterrows(), 1):
            print(f"  {i}. {r['year']} Q{r['question_number']} | title={r['question_title']!r}")
            print(f"     {str(r['error_text'])[:180]}")
