...
Usage:
    python scripts/run_mapping_all_years.py

Reads each question_topic_map_YYYY_v2.csv, re-maps with the frozen
artefacts, and writes the file back.
"""
...
def map_year(year, sig_df, token_map, struct_rules):
    path = MAP_DIR / str(year) / f"question_topic_map_{year}_v2.csv"
    if not path.exists():
...
def main():
    ...
    print(f"Loaded: {len(sig_df)} signatures, "
          f"{len(struct_rules)} struct rules, "
          f"{len(token_map)} lexicon tokens")
    print()

    for year in ALL_YEARS:
        n = map_year(year, sig_df, token_map, struct_rules)
        if n:
            print(f"{year}: {n} rows re-mapped")
